...
def sequential_window_dataset(
    series: np.ndarray, batch_size: int, n_past: int, n_future: int, shift: int
) -> tf.data.Dataset:
    """_summary_
    ...
    return (
        ...
        )  # window features (add one for future label)
        .flat_map(
            lambda w: w.batch(n_past + n_future)
        )  # window features (add one for future label)
        .map(
...
if __name__ == "__main__":
    ...
    df = yf.Ticker("AAPL").history(interval="1d", start="2020-01-01", end="2024-01-30")
    ...
    model = build_bidirec_lstm_model(
        n_past=N_PAST, n_features=N_FEATURES, batch_size=BATCH_SIZE
    )
    ...
    history = model.fit(
        train_set, epochs=EPOCHS, validation_data=val_set, callbacks=[early_stopping]
    )
